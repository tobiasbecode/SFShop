 ...
 * @suppress {checkTypes} checked by tsc
 */
class SliderElementDirective extends JqLiteWrapper {
    /**
     * @param {?} elemRef
     * @param {?} renderer
     */
...
class SliderComponent {
    ...
    subscribeResizeObserver() {
        if (CompatibilityHelper.isResizeObserverAvailable()) {
            this.resizeObserver = new ResizeObserver(() => this.calcViewDimensions());
    ...
    shFloorCeil() {
        ...
        const /** @type {?} */ isMinLabAtFloor = this.isLabelBelowFloorLab(this.minHandleLabelElement);
        const /** @type {?} */ isMinLabAtCeil = this.isLabelAboveCeilLab(this.minHandleLabelElement);
        const /** @type {?} */ isMaxLabAtCeil = this.isLabelAboveCeilLab(this.maxHandleLabelElement);
        const /** @type {?} */ isCmbLabAtFloor = this.isLabelBelowFloorLab(this.combinedLabelElement);
        const /** @type {?} */ isCmbLabAtCeil = this.isLabelAboveCeilLab(this.combinedLabelElement);
        if (isMinLabAtFloor) {
            flHidden = true;
    ...
    bindEvents() {
        ...
        if (!this.viewOptions.onlyBindHandles) {
            this.selectionBarElement.on('mousedown', (event) => this.onBarStart(draggableRange, event, true, true, true));
        }
        if (this.viewOptions.draggableRangeOnly) {
            this.minHandleElement.on('mousedown', (event) => this.onBarStart(draggableRange, event, true, true));
            this.maxHandleElement.on('mousedown', (event) => this.onBarStart(draggableRange, event, true, true));
        }
        else {
        ...
        if (!this.viewOptions.onlyBindHandles) {
            this.selectionBarElement.onPassive('touchstart', (event) => this.onBarStart(draggableRange, event, true, true));
        }
        if (this.viewOptions.draggableRangeOnly) {
            this.minHandleElement.onPassive('touchstart', (event) => this.onBarStart(draggableRange, event, true, true));
            this.maxHandleElement.onPassive('touchstart', (event) => this.onBarStart(draggableRange, event, true, true));
        }
        else {
     ...
     * @return {?}
     */
    onBarStart(draggableRange, event, bindMove, bindEnd, simulateImmediateMove, simulateImmediateEnd) {
        if (draggableRange) {
